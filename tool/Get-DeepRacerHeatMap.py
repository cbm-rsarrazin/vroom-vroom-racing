...
def get_string_path_data(loggroupname, logstreamname, starttimeepoch, endtimeepoch):
    ...
    for event in events:
        ...
        vehicle_view_distance = float(commasplit[16].split(':')[1].strip())
        vehicle_speed = float(commasplit[17].split(':')[1].strip())
...
#         vehicle_y = coord['vehicle_y']
#
#         plt.scatter(vehicle_x, vehicle_y, color='blue', alpha=0.04)
